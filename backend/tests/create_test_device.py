...
async def create_test_device():
    async with async_session_factory() as db:
        ...
        if not plant_type:
            logger.error("PlantType 'Kim Tiền' not found in database. Please run python seed.py first.")
            return

        logger.info(f"Found PlantType 'Kim Tiền' with ID: {plant_type.id}. Updating ideal thresholds for real-world logs...")
        ...
        if not rank:
            logger.error("RankConfig for order 1 not found in database. Please run python seed.py first.")
            return
        ...
        if not is_match:
            logger.info("Hardcoded verify_hash does not match '123456'. Generating a correct hash...")
            verify_hash = bcrypt.hashpw("123456".encode("utf-8"), bcrypt.gensalt()).decode("utf-8")
        ...
        if not device:
            ...
            logger.info(f"Found existing device with ID: {device.id}. Updating state...")
